...
HRESULT CModuleSafety::SetObjStatePS(PTComInitDataHdr pInitData)
{
	...
	m_SafetyController.m_Trace = &m_Trace;
...
HRESULT CModuleSafety::SetObjStateSO()
{
	...
	{
		CSafetyController::m_pEventLogger = m_spEventLogger;
		CSafetyController::m_pMessage = m_spMessage;
	}
...
HRESULT CModuleSafety::CycleUpdate(ITcTask* ipTask, ITcUnknown* ipCaller, ULONG_PTR context)
{
	...
	if (!m_IsRpcReady)
	{
		...
		{
			m_Trace.Log(tlError, FLEAVEA "Level tlError : Safety module failed to initialize rpc.");
