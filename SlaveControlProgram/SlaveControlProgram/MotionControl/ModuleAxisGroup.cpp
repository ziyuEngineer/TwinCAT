...
HRESULT CModuleAxisGroup::RequestRecovery(bool is_entered)
{
	HRESULT hr = S_OK;
	m_csInstance.EnterCriticalSection();
	...
	if (is_entered)
	{
		m_AxisGroupStateMachine->dispatch(EventAxisGroupEnterRecoveryState());
	}
	else
	{
		m_AxisGroupStateMachine->dispatch(EventAxisGroupExitRecoveryState());
	}
