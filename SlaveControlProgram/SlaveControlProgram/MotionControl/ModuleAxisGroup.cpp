...
HRESULT CModuleAxisGroup::SetObjStateSO()
{
	...
	{
		CAxisGroupController::m_pEventLogger = m_spEventLogger;
		CAxisGroupController::m_pMessage = m_spMessage;
	}
...
HRESULT CModuleAxisGroup::ParseFileAndAssignValue(const char* xml_axis_group)
{
	...
	while (p_axes) {
		...
		while (p_axis) {
			...
			m_Parameter.AxisPosUpperLimit[total_driver_num] = atof(ptr->GetText());

			ptr = p_axis->FirstChildElement("LowerPosLimit");
			m_Parameter.AxisPosLowerLimit[total_driver_num] = atof(ptr->GetText());
...
HRESULT CModuleAxisGroup::SetAxisVelLoopKp(SHORT axis, double value)
{
	HRESULT hr = S_OK;

	hr = m_AxisGroupController.SetSingleAxisVelLoopKp(axis, value);

	return hr;
}

HRESULT CModuleAxisGroup::SetAxisVelLoopTn(SHORT axis, double value)
{
	HRESULT hr = S_OK;

	hr = m_AxisGroupController.SetSingleAxisVelLoopTn(axis, value);

	return hr;
}

HRESULT CModuleAxisGroup::SetAxisPosLoopKv(SHORT axis, double value)
{
	HRESULT hr = S_OK;

	hr = m_AxisGroupController.SetSingleAxisPosLoopKv(axis, value);

	return hr;
}

HRESULT CModuleAxisGroup::GetAxisVelLoopKp(SHORT axis, double& value)
{
	HRESULT hr = S_OK;

	hr = m_AxisGroupController.GetSingleAxisVelLoopKp(axis, value);

	return hr;
}

HRESULT CModuleAxisGroup::GetAxisVelLoopTn(SHORT axis, double& value)
{
	HRESULT hr = S_OK;

	hr = m_AxisGroupController.GetSingleAxisVelLoopTn(axis, value);

	return hr;
}

HRESULT CModuleAxisGroup::GetAxisPosLoopKv(SHORT axis, double& value)
{
	HRESULT hr = S_OK;

	hr = m_AxisGroupController.GetSingleAxisPosLoopKv(axis, value);

	return hr;
}

HRESULT CModuleAxisGroup::RequestMovingToTargetPos(bool* axis, double* pos)
{
	HRESULT hr = S_OK;
		
	m_csInstance.EnterCriticalSection();
		
	EventAxisGroupPositioning event;
	for (int i = 0; i < kMaxAxisNum; i++)
	{
		event.enabled_axis[i] = axis[i];
		event.target[i] = pos[i];
	}
	
	memcpy(&m_Outputs.TestEnabled, &event.enabled_axis, sizeof(bool[5]));
	memcpy(&m_Outputs.TestTarget, &event.target, sizeof(double[5]));
	m_AxisGroupStateMachine->dispatch(event);
